...
/**
 * Credit: <a href="https://github.com/RRS-9747/HeadDrop">HeadDrop Plugin</a>
 * Note: not all heads are included here, only a selection of them.
 */
@Getter
public enum EntityHeads {
    ...
    EntityHeads(String name, String sound, String headHash) {
        this.name = name;
        this.sound = sound;
    ...
    public static ItemStack fromEntityType(EntityType type) {
        if (type == EntityType.ZOMBIE) {
            return ItemBuilder.from(Material.ZOMBIE_HEAD).build();
        } else if (type == EntityType.SKELETON) {
            return ItemBuilder.from(Material.SKELETON_SKULL).build();
        } else if (type == EntityType.CREEPER) {
            return ItemBuilder.from(Material.CREEPER_HEAD).build();
        } else if (type == EntityType.ENDER_DRAGON) {
            return ItemBuilder.from(Material.DRAGON_HEAD).build();
        } else if (type == EntityType.PIGLIN) {
            return ItemBuilder.from(Material.PIGLIN_HEAD).build();
    ...
    public ItemStack getSkull() {
        return ItemBuilder.skull().texture(headHash, headUUID).build();
    }
}
