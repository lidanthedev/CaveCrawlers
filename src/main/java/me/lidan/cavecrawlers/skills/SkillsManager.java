...
public class SkillsManager extends ConfigLoader<SkillInfo> implements SkillsAPI {
    ...
    public void giveXp(Player player, SkillInfo skillType, double xp, boolean showMessage) {
        ...
        if (showMessage) {
            Component component = MiniMessageUtils.miniMessageString("<dark_aqua>+<xp> <skill-name> (<xp-percent>%)", Map.of("xp", StringUtils.valueOf(event.getXpGained()), "skill-name", skillName, "xp-percent", String.valueOf(Math.floor(skill.getXp() / skill.getXpToLevel() * 1000d) / 10d)));
